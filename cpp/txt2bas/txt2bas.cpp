...
namespace txt2bas {
    ...
    std::vector<uint8_t> BasConverter::ParseLine(int lineNum, const std::string& text) {
        ...
        bool expectCommand = true; // Tracks if we are at a point where a command is valid

        for (size_t i = 0; i < text.length(); i++) {
            
            // Maintain command state through spaces and tabs
            if (text[i] == ' ' || text[i] == '\t') {
                lineData.push_back(static_cast<uint8_t>(text[i]));
                continue;
            }

            // DOT COMMAND LOGIC (.run, .cd, etc.)
            // If we expect a command and find a dot:
            if (expectCommand && text[i] == '.') {
                // Ensure it isn't a floating point number like '.5'
                if (i + 1 < text.length() && std::isdigit(static_cast<unsigned char>(text[i+1]))) {
                    // Let it fall through to the numeric logic below
                } else {
                    // It is a NextZXOS Dot Command!
                    // Treat the entire rest of the line as literal ASCII text.
            ...
            // Strings
            if (text[i] == '"') {
                expectCommand = false; // Inside/after a string, no commands expected
            ...
            if (std::isdigit(static_cast<unsigned char>(text[i])) || (text[i] == '.' && i + 1 < text.length() && std::isdigit(static_cast<unsigned char>(text[i+1])))) {
                expectCommand = false; // After a number, no commands expected
                std::string numStr = "";
                size_t j = i;
            ...
            for (const std::string& k : _sortedKeys) {
                ...
                if (token == 0xEA) { // REM - copy literal
                    ...
                    // Check if this token resets the command expectation
            ...
            lineData.push_back(static_cast<uint8_t>(text[i]));
            if (text[i] == ':') {
                expectCommand = true; // A colon allows a new command
            } else {
                expectCommand = false;
            }
        }
