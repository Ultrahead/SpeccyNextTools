...
namespace txt2bas {
    ...
    std::vector<uint8_t> BasConverter::ParseLine(int lineNum, const std::string& text) {
        ...
        for (size_t i = 0; i < text.length(); i++) {
            ...
            // 2. COMMENT HANDLING: Strict check for ';comment' idiom
            ...
            // 3. NUMBERS (Identifier-Aware Parsing)
            ...
            if (std::isdigit(static_cast<unsigned char>(text[i])) || 
               (text[i] == '.' && i + 1 < text.length() && std::isdigit(static_cast<unsigned char>(text[i+1])))) {
                ...
                int back = static_cast<int>(i) - 1;
                while (back >= 0 && std::isdigit(static_cast<unsigned char>(text[back]))) back--;
                
                if (back >= 0 && (std::isalpha(static_cast<unsigned char>(text[back])) || text[back] == '_')) {
                    isNumberStart = false;
                }
            }
